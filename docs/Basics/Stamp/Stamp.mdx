...
You can use RGBA values sampled from footprints directly.
...
If necessary to render square footprints, I guess experienced graphics developers can instantly get some ideas on
new approaches to placing the vertices and determining stamp positions.
But the ideas may not be good for performance and have complex logic to implement.
...
Instead of offset vertices by $r_i\tan\frac{\theta}{2}$ along the normal direction, we just offset $r_i$ value,
as the figure below shows.
![square](./stamp-square.png)
We put the radius value $r_0$ to the left two vertices, $r_1$ to the right two vertices
and let fragment shader interpolate the values for us.
Then we get an interpolated radius value $r_p$ in the fragment shader.
You can prove this $r_p$ is the stroke radius value across the current pixel.
We assume the whole stroke is uni-radius and program the shader it with $r_p$.
Each stamp is a square, so we don't need to discard the corners.
