...
#### Why is geometry shader elegant?

The variable `vao` is created exactly the same like when drawing a line strip.
...
When mocking geometry shader's behavior with instanced rendering, everything gets obscure.
Sadly, it's hard for us graphics engineers to enjoy this elegance since Apple's Metal API.
...
Writing maintainable and easy-to-understand code is much more important.
...
It's obvious that we are not dealing with transparency correctly.
...
When the small circle is very close to the big one, we get large `normalTanValue` value and invoked triangles will cover large areas of pixels,
...
The two corner `if` conditions happened with the same situation.
The corner case happens when the radius changes in a short distance between points.
The corner case rarely happens for stroke in 2D space but commonly in 3D.
I will introduce the solution together with 3D stroke rendering.
