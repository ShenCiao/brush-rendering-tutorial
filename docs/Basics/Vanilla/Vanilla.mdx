...
In the diagram below, we select an edge with two consecutive vertices, labeled as $v_i$ and $v_{i+1}$, then render a capsule on it.
...
The most elegant one is to use geometry shader.
...
To render a variable radius stroke, we are given a polyline with radius value at each point.
The process is basically the same as uni-radius stroke:
...
The vertices form a trapezoid rather than the rectangle for uni radius.
We need to solve a few extra geometric parameters to place this trapezoid.
