...
Previously, we touched upon alpha compositing formula in the Vanilla -> Opacity section. In this section, we delve deeper to offer a more detailed derivation of alpha blending through two distinct approaches.

## Via Compositing Algebra

Let's define the color of an object as $a=(C_a,A_a)$, where $C_a$ represents the RGB color components, $A_a$ represents the alpha value. If the object is opaque ($A_a=1$), the color is represented by $\widehat{a}$. Otherwise when the object has variable transparency ($A_x \neq 1$), the color is simply represented by $a$.
...
$$
A_oC_o+(1-A_o)C_c =a \odot(A_bC_b+(1-A_b)C_c)
$$
...
## Via Transparency
Opacity and transparency are critical properties in 2D graphics, influencing how objects are visually percieved. Opacity is the measure of the extent to which an object blocks light, while transparency is its inverse, representing the object's see-through quality. To quantify these, consider the alpha value of an pixel, $A \in [0.0, 1.0]$. The transparency $T$ of this pixel is then calculated as:

$$
T = 1 - A \tag{2}
$$

To calculate the result transparency $T_o$ by composing two objects with transparencies $T_a$ and $T_b$ separately, we can simply multiply them:
$$T_o = T_a \times T_b$$
